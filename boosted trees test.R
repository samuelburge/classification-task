...
for (b in 1:bootStraps){
  ...
  picked = sample(seq_len(nrow(dataset)),size = floor(sample_size*nrow(dataset)))  
...
summary = resultsDataFrame %>%
  ...
  summarize(meanAccuracy = mean(validationAccuracy), SD = sd(validationAccuracy)) %>%
...
names<- c('bootStrapNum','testAccuracy','validationAccuracy')
resultsDataFrame <- data.frame(matrix(ncol = length(names), nrow = 0)) 
colnames(resultsDataFrame) <- names
...
for (b in 1:bootStraps){
  ...
  responseVector<-boost.outer.pred>=0.5
  innerresponseVector<-boost.inner.pred>=0.5
  accuracy<-(nrow(outerSplit)-sum(abs(responseVector-outerSplit$y)))/nrow(outerSplit)
  inneraccuracy<-(nrow(innerSplit)-sum(abs(innerresponseVector-innerSplit$y)))/nrow(innerSplit)
  strapResult<-c(b,inneraccuracy,accuracy)
  resultsDataFrame<-rbind(resultsDataFrame,strapResult)
  print(paste(b,'complete'))
}

newDataFrame<-read.csv('resultsDataFrame.csv')
